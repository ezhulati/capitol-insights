...
function processInlineFormatting(text: string): string {
  // Process bold text (** **) with proper styling
  return text.replace(/\*\*(.*?)\*\*/g, '$1');
}

// Define possible return types for content blocks
type ContentBlock = 
  | { type: string, content: string, key: number, id?: string }
  | { type: string, items: string[], key: number };
...
export function renderMarkdown(content: string | any): ContentBlock[] {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (trimmed.startsWith('# ')) {
      const headingContent = processInlineFormatting(trimmed.substring(2));
      result.push({ 
        type: 'h1', 
        content: headingContent, 
        key: key++,
        id: headingContent.toLowerCase().replace(/<[^>]*>/g, '').replace(/[^\w\s]/g, '').replace(/\s+/g, '-')
      });
    } else if (trimmed.startsWith('## ')) {
      const headingContent = processInlineFormatting(trimmed.substring(3));
      result.push({ 
        type: 'h2', 
        content: headingContent, 
        key: key++,
      ...
      const headingContent = processInlineFormatting(trimmed.substring(4));
      result.push({ 
        type: 'h3', 
        content: headingContent, 
        key: key++,
      ...
      const items = trimmed.split('\n').map(item => {
        ...
        return processInlineFormatting(listItem);
      });
      result.push({ type: 'ul', items, key: key++ });
      ...
      result.push({ type: 'ol', items, key: key++ });
    } else {
      // Process paragraphs with inline formatting
      result.push({ 
        type: 'p', 
        content: processInlineFormatting(trimmed), 
        key: key++ 
      });
    }
  }
