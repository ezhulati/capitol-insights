...
interface ImprovedLeadMagnetFormProps {
  ...
  ctaText: string;
  downloadUrl: string;
  pdfUrl: string; // Direct URL to the HTML document
  className?: string;
}

const ImprovedLeadMagnetForm: React.FC<ImprovedLeadMagnetFormProps> = ({
  title,
  subtitle,
  description,
  bulletPoints,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First open the document in a new tab
      openDocument();
      
      // Then send lead data to backend or email service
      ...
      // Even if the API call fails, we still want to show success and 
      // ensure the user can get the download
      setIsSubmitting(false);
      setIsSubmitted(true);
      
      // Try to open the document again if the first attempt failed
      openDocument();
    }
  };
        ...
        <div className="bg-navy-50 p-6 md:p-8 md:w-2/5">
          {!isSubmitted ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-slate-600 mb-4">Your guide should open in a new tab.</p>
...
export default ImprovedLeadMagnetForm;
